...
const CONFIG = {
  ...
  SUNSHINE_PORT: 47990,
  MONITOR_KEY: 'fa0d491cc02ed2e2d1a9446755a637e0a933e4348b280f461079e790d01a5ad4',
};
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = lib.request({
      ...
      headers: {
        ...
        'x-monitor-key': CONFIG.MONITOR_KEY,
...
// ─── 가상 USB HID 마우스 + 키보드 (USB/IP 루프백, 통합 서버) ─────────────────
const hidServer = require('./usbip-hid-server');
let virtualMouseAttached = false;
let virtualKeyboardAttached = false;

function startVirtualMouse() {
  if (virtualMouseAttached) return;

  // 1) HID 서버 시작 (localhost:3240)
  hidServer.startServer(3240);

  // 2) 잠시 대기 후 attach (가상 USB 마우스 생성)
  setTimeout(() => {
    if (!USBIP_EXE) {
      console.log('[VirtualMouse] usbip.exe 없음 — 가상 마우스 생성 불가');
      return;
    }
    console.log('[VirtualMouse] 가상 마우스 attach 시도...');
    exec(`"${USBIP_EXE}" attach -r 127.0.0.1 -b 1-1`, { windowsHide: true, timeout: 10000 }, (err, stdout, stderr) => {
      if (err) {
        console.log(`[VirtualMouse] attach 실패: ${err.message}`);
        if (stderr) console.log(`[VirtualMouse] stderr: ${stderr.trim()}`);
      } else {
        virtualMouseAttached = true;
        console.log('[VirtualMouse] 가상 USB 마우스 생성 완료 ✅ (Logitech G102)');
      }
    });
  }, 1000);
}
...
function startVirtualKeyboard() {
  if (virtualKeyboardAttached) return;

  // 통합 서버는 startVirtualMouse()에서 이미 시작됨 (포트 3240)
  setTimeout(() => {
    if (!USBIP_EXE) {
      console.log('[VirtualKB] usbip.exe 없음 — 가상 키보드 생성 불가');
      return;
    }
    console.log('[VirtualKB] 가상 키보드 attach 시도...');
    exec(`"${USBIP_EXE}" attach -r 127.0.0.1 -b 1-2`, { windowsHide: true, timeout: 10000 }, (err, stdout, stderr) => {
      if (err) {
        console.log(`[VirtualKB] attach 실패: ${err.message}`);
        if (stderr) console.log(`[VirtualKB] stderr: ${stderr.trim()}`);
      } else {
        virtualKeyboardAttached = true;
        console.log('[VirtualKB] 가상 USB 키보드 생성 완료 ✅ (Logitech K120)');
        setTimeout(() => {
          startKeyboardForwarder();
          startGameGuardWatcher();
        }, 2000);
      }
    });
  }, 1000);
...
function stopVirtualMouse() {
  stopGameGuardWatcher();
  stopMouseForwarder();
  if (!virtualMouseAttached) return;
  if (USBIP_EXE) {
    try {
      execSync(`"${USBIP_EXE}" detach -p 0`, { windowsHide: true, timeout: 5000 });
    } catch (_) {}
  }
  hidServer.stopServer();
  virtualMouseAttached = false;
  console.log('[VirtualMouse] 가상 마우스 제거');
}

function stopVirtualKeyboard() {
  stopKeyboardForwarder();
  if (!virtualKeyboardAttached) return;
  if (USBIP_EXE) {
    try {
      execSync(`"${USBIP_EXE}" detach -p 1`, { windowsHide: true, timeout: 5000 });
    } catch (_) {}
  }
  // 통합 서버는 stopVirtualMouse()에서 중지
  virtualKeyboardAttached = false;
  console.log('[VirtualKB] 가상 키보드 제거');
}

// ─── 마우스 입력 포워딩 (Sunshine 입력 → 가상 USB HID) ──────────────────────
let mouseForwarderProcess = null;

function startMouseForwarder() {
  if (mouseForwarderProcess) return;
  if (!virtualMouseAttached) return;

  // C# 코드를 임시 파일로 저장 후 실행 (인라인 문자열 깨짐 방지)
  const psScript = path.join(os.tmpdir(), 'mouse-forwarder.ps1');
...
public class MouseForwarder {
    ...
    public static void Run() {
        ...
        while (true) {
            int buttons = 0;
            if ((GetAsyncKeyState(1) & 0x8000) != 0) buttons |= 1;
            if ((GetAsyncKeyState(2) & 0x8000) != 0) buttons |= 2;
            if ((GetAsyncKeyState(4) & 0x8000) != 0) buttons |= 4;

            if (buttons != lastButtons) {
                Console.WriteLine(buttons + ",0,0,0");
                Console.Out.Flush();
                lastButtons = buttons;
            }
...
`, 'utf8');
  ...
  console.log('[MouseFwd] 마우스 포워딩 시작');
  mouseForwarderProcess = execFile('powershell.exe', [
    '-NonInteractive', '-WindowStyle', 'Hidden', '-ExecutionPolicy', 'Bypass', '-File', psScript
  ], { windowsHide: true });

  mouseForwarderProcess.stdout.on('data', (data) => {
    const lines = data.toString().trim().split(/\r?\n/);
    for (const line of lines) {
      const parts = line.split(',');
      if (parts.length < 4) continue;
      const buttons = parseInt(parts[0]) || 0;
      const dx = parseInt(parts[1]) || 0;
      const dy = parseInt(parts[2]) || 0;
      const wheel = parseInt(parts[3]) || 0;
      const sent = hidServer.sendMouseReport(buttons, dx, dy, wheel);
      console.log(`[MouseFwd] 버튼=${buttons} → HID ${sent ? '전송' : '실패(pendingIn 없음)'}`);
    }
  });

  mouseForwarderProcess.stderr.on('data', (d) => {
    const msg = d.toString().trim();
    if (msg) console.log('[MouseFwd] stderr:', msg);
  });

  mouseForwarderProcess.on('exit', (code) => {
    console.log(`[MouseFwd] 종료 (code=${code})`);
    mouseForwarderProcess = null;
  });
}

function stopMouseForwarder() {
  if (mouseForwarderProcess) {
    mouseForwarderProcess.kill();
    mouseForwarderProcess = null;
    console.log('[MouseFwd] 포워딩 중지');
  }
}

// ─── GameGuard 감지 → 마우스 HID 포워딩 자동 ON/OFF ─────────────────────────
let gameGuardWatcherTimer = null;
let gameGuardActive = false;
let ggDebugCount = 0;

function startGameGuardWatcher() {
  if (gameGuardWatcherTimer) return;
  console.log('[GG-Watch] GameGuard 감시 시작');

  gameGuardWatcherTimer = setInterval(() => {
    execFile('powershell.exe', [
      '-NonInteractive', '-WindowStyle', 'Hidden', '-Command',
      'Get-Process | Where-Object { $_.MainWindowTitle -ne "" } | Select-Object -ExpandProperty MainWindowTitle | Out-String'
    ], { windowsHide: true, timeout: 5000 }, (err, stdout) => {
      if (err) { console.log('[GG-Watch] 에러:', err.message); return; }
      const out = (stdout || '').toLowerCase();
      // 윈도우 타이틀로 게임 감지 (리니지, lineage 등)
      const running = out.includes('lineage') || out.includes('리니지') ||
        out.includes('lostark') || out.includes('로스트아크') ||
        out.includes('maplestory') || out.includes('메이플') ||
        out.includes('fc online') || out.includes('fc 온라인') || out.includes('fifa') ||
        out.includes('던전앤파이터') || out.includes('dungeon') ||
        out.includes('서든어택') || out.includes('suddenattack') ||
        out.includes('배틀그라운드') || out.includes('pubg') ||
        out.includes('검은사막') || out.includes('black desert') ||
        out.includes('발로란트') || out.includes('valorant') ||
        out.includes('오버워치') || out.includes('overwatch');
      // 디버그: 첫 3회만 출력
      ggDebugCount++;
      if (ggDebugCount <= 3) {
        const titles = out.split('\n').map(s => s.trim()).filter(Boolean).join(', ');
        console.log(`[GG-Watch] 윈도우: ${titles.substring(0, 300)}`);
        console.log(`[GG-Watch] 게임 감지: ${running}`);
      }

      if (running && !gameGuardActive) {
        gameGuardActive = true;
        console.log('[GG-Watch] 게임 감지 → 마우스 HID 포워딩 ON');
      } else if (!running && gameGuardActive) {
        gameGuardActive = false;
        console.log('[GG-Watch] 게임 종료 → 마우스 HID 포워딩 OFF');
      }
    });
  }, 3000);
}

function stopGameGuardWatcher() {
  if (gameGuardWatcherTimer) {
    clearInterval(gameGuardWatcherTimer);
    gameGuardWatcherTimer = null;
  }
  gameGuardActive = false;
}

// ─── 키보드 입력 포워딩 (Sunshine 입력 → 가상 USB HID 키보드) ────────────────
let keyboardForwarderProcess = null;

// VK → HID Usage Code 매핑 테이블 (C# 배열로 변환)
const VK_TO_HID = {
  // A-Z
  ...
  0x55: 0x18, 0x56: 0x19, 0x57: 0x1A, 0x58: 0x1B, 0x59: 0x1C, 0x5A: 0x1D,
  // 1-9, 0
  ...
  0x6D: 0x56, // Numpad -
  0x6B: 0x57, // Numpad +
  0x6C: 0x58, // Numpad Enter (VK_SEPARATOR — 실제로는 아래에서 처리)
  0x61: 0x59, // Numpad 1
  0x62: 0x5A, // Numpad 2
  0x63: 0x5B, // Numpad 3
  0x64: 0x5C, // Numpad 4
...
// 폴링할 VK 코드 목록 (modifier 제외)
const VK_POLL_LIST = Object.keys(VK_TO_HID).map(Number);

// C# 코드에서 사용할 VK 목록 문자열
const VK_LIST_STR = VK_POLL_LIST.join(',');

function startKeyboardForwarder() {
  if (keyboardForwarderProcess) return;
  if (!virtualKeyboardAttached) return;

  const psScript = path.join(os.tmpdir(), 'keyboard-forwarder.ps1');
  fs.writeFileSync(psScript, `
Add-Type @'
using System;
using System.Runtime.InteropServices;
...
public class KeyboardForwarder {
    [DllImport("user32.dll")]
    public static extern short GetAsyncKeyState(int vKey);

    public static void Run() {
        int[] vkList = new int[] { ${VK_LIST_STR} };
        int[] lastState = new int[vkList.Length];
        int lastMod = 0;
        int lastMb = 0;

        while (true) {
            int mod = 0;
            if ((GetAsyncKeyState(0xA2) & 0x8000) != 0) mod |= 1;
            if ((GetAsyncKeyState(0xA0) & 0x8000) != 0) mod |= 2;
            if ((GetAsyncKeyState(0xA4) & 0x8000) != 0) mod |= 4;
            if ((GetAsyncKeyState(0x5B) & 0x8000) != 0) mod |= 8;
            if ((GetAsyncKeyState(0xA3) & 0x8000) != 0) mod |= 16;
            if ((GetAsyncKeyState(0xA1) & 0x8000) != 0) mod |= 32;
            if ((GetAsyncKeyState(0xA5) & 0x8000) != 0) mod |= 64;
            if ((GetAsyncKeyState(0x5C) & 0x8000) != 0) mod |= 128;

            // 마우스 버튼
            int mb = 0;
            if ((GetAsyncKeyState(1) & 0x8000) != 0) mb |= 1;
            if ((GetAsyncKeyState(2) & 0x8000) != 0) mb |= 2;
            if ((GetAsyncKeyState(4) & 0x8000) != 0) mb |= 4;

            bool changed = (mod != lastMod) || (mb != lastMb);
            string pressed = "";
            int count = 0;

            for (int i = 0; i < vkList.Length; i++) {
                int state = (GetAsyncKeyState(vkList[i]) & 0x8000) != 0 ? 1 : 0;
                if (state != lastState[i]) changed = true;
                lastState[i] = state;
                if (state == 1 && count < 6) {
            ...
            if (changed) {
                Console.WriteLine(mod + "|" + pressed + "|" + mb);
                Console.Out.Flush();
                lastMod = mod;
                lastMb = mb;
            }
...
'@
[KeyboardForwarder]::Run()
`, 'utf8');

  console.log('[KeyFwd] 키보드 포워딩 시작');
  keyboardForwarderProcess = execFile('powershell.exe', [
    '-NonInteractive', '-WindowStyle', 'Hidden', '-ExecutionPolicy', 'Bypass', '-File', psScript
  ], { windowsHide: true });

  keyboardForwarderProcess.stdout.on('data', (data) => {
    const lines = data.toString().trim().split(/\r?\n/);
    for (const line of lines) {
      const parts = line.split('|');
      if (parts.length < 3) continue;
      const modifiers = parseInt(parts[0]) || 0;
      const vkList = parts[1] ? parts[1].split(',').map(Number).filter(v => !isNaN(v)) : [];
      const keys = vkList.map(vk => VK_TO_HID[vk] || 0).filter(v => v > 0);
      const mouseButtons = parseInt(parts[2]) || 0;

      // 키보드는 항상 전달
      hidServer.sendKeyboardReport(modifiers, keys);

      // 마우스 버튼은 게임 실행 중일 때만 HID로 전달 (데스크톱 더블클릭 보호)
      if (gameGuardActive) {
        hidServer.sendMouseReport(mouseButtons, 0, 0, 0);
      }
    }
  });

  keyboardForwarderProcess.stderr.on('data', (d) => {
    const msg = d.toString().trim();
    if (msg) console.log('[KeyFwd] stderr:', msg);
  });

  keyboardForwarderProcess.on('exit', (code) => {
    console.log(`[KeyFwd] 종료 (code=${code})`);
    keyboardForwarderProcess = null;
  });
}

function stopKeyboardForwarder() {
  if (keyboardForwarderProcess) {
    keyboardForwarderProcess.kill();
    keyboardForwarderProcess = null;
    console.log('[KeyFwd] 포워딩 중지');
  }
}
...
async function main() {
  ...
  // 가상 USB 마우스 + 키보드 시작 (GameGuard 우회용)
  startVirtualMouse();
  setTimeout(() => startVirtualKeyboard(), 3000); // 마우스 attach 후 키보드 attach
