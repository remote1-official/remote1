...
function launchMoonlight(host, settings) {
  ...
  // Moonlight 오버레이 팁("tip: press...") 비활성화 — 레지스트리에 미리 설정
  try {
    execSync(
      'reg add "HKCU\\Software\\Moonlight Game Streaming Project\\Moonlight" /v tipsdismissed /t REG_DWORD /d 1 /f',
  ...
  // 프로세스 시작 직후부터 아이콘 교체 루프 시작 (모든 창 단계에서 적용)
  startIconLoop(moonlightProcess.pid);
}
...
// 런타임 아이콘 교체 (PowerShell — Moonlight이 내부에서 아이콘을 세팅하므로 덮어써야 함)
const ICON_ICO = path.join(__dirname, '..', 'assets', 'icon.ico');
// 한글 경로 문제 방지: temp에 ico 복사
const ICON_TEMP = path.join(app.getPath('temp'), 'r1_icon.ico');
try { fs.copyFileSync(ICON_ICO, ICON_TEMP); } catch (_) {}
const ICON_PS_PATH = ICON_TEMP.replace(/\\/g, '\\\\');

// PID 기반 아이콘 교체 루프 (프로세스 시작 직후부터 모든 창에 반복 적용)
let iconLoopProcess = null;
function startIconLoop(pid) {
  stopIconLoop();
  const loopScript = [
    'Add-Type @"',
    'using System;',
    ...
    '  public static void Run(int pid, string icoPath) {',
    '    IntPtr big = LoadImage(IntPtr.Zero, icoPath, 1, 32, 32, 0x10);',
    '    IntPtr small = LoadImage(IntPtr.Zero, icoPath, 1, 16, 16, 0x10);',
    '    if (big == IntPtr.Zero && small == IntPtr.Zero) return;',
    '    for (int i = 0; i < 1500; i++) {',
    '      try {',
    '        var p = Process.GetProcessById(pid);',
    '        if (p.MainWindowHandle != IntPtr.Zero) {',
    '          if (big != IntPtr.Zero) SendMessage(p.MainWindowHandle, 0x0080, (IntPtr)1, big);',
    '          if (small != IntPtr.Zero) SendMessage(p.MainWindowHandle, 0x0080, IntPtr.Zero, small);',
    '          SetWindowText(p.MainWindowHandle, "REMOTE1");',
    '        }',
    '      } catch { break; }',
    ...
    '}',
    '"@',
    `[IcoLoop]::Run(${pid}, "${ICON_PS_PATH}")`,
  ].join('\n');
  const scriptPath = path.join(app.getPath('temp'), 'r1_iconloop.ps1');
  const bom = Buffer.from([0xEF, 0xBB, 0xBF]);
  fs.writeFileSync(scriptPath, Buffer.concat([bom, Buffer.from(loopScript, 'utf8')]));
  iconLoopProcess = execFile('powershell.exe',
    ['-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', scriptPath],
    { windowsHide: true, timeout: 35000 },
    () => { try { fs.unlinkSync(scriptPath); } catch (_) {} }
  );
}
function stopIconLoop() {
  if (iconLoopProcess) {
    try { iconLoopProcess.kill(); } catch (_) {}
    iconLoopProcess = null;
  }
}

function setRemoteIcon(hwnd) {
  const script = [
    'Add-Type @"',
    'using System;',
    'using System.Runtime.InteropServices;',
    ...
    '}',
    '"@',
    `[ICO]::SetIcon([IntPtr]${hwnd}, "${ICON_PS_PATH}")`,
  ].join('\n');
  const scriptPath = path.join(app.getPath('temp'), `r1_seticon_${Date.now()}.ps1`);
  // UTF-8 BOM으로 저장 (한글 경로 지원)
  const bom = Buffer.from([0xEF, 0xBB, 0xBF]);
  fs.writeFileSync(scriptPath, Buffer.concat([bom, Buffer.from(script, 'utf8')]));
  execFile('powershell.exe',
    ['-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', scriptPath],
    { windowsHide: true, timeout: 5000 },
    (err) => {
      if (err) console.error('[SetIcon] PS error:', err.message);
      try { fs.unlinkSync(scriptPath); } catch (_) {}
    }
  );
}
...
function startMoonlightTitlePoll() {
  ...
  moonlightTitlePollTimer = setInterval(() => {
    ...
    execFile('powershell.exe', [
      ...
      } else if (moonlightTitlePhase === 'waiting' || moonlightTitlePhase === 'loading') {
        // 로딩 중 — 매 폴링마다 새 창이 뜰 수 있으므로 항상 제목+아이콘 적용
        moonlightTitlePhase = 'loading';
        setMoonlightTitle(hwnd, 'REMOTE1 - 접속중');
        setRemoteIcon(hwnd);
        console.log('[TitlePoll] Loading phase, title+icon set (was:', title, ')');
      }
    });
  }, 300);
}

let iconKeepAliveTimers = [];
function startIconKeepAlive(hwnd) {
  stopIconKeepAlive();
  [500, 1500, 3000, 6000, 10000].forEach(delay => {
    iconKeepAliveTimers.push(setTimeout(() => {
      if (moonlightProcess) setRemoteIcon(hwnd);
    }, delay));
  });
}
function stopIconKeepAlive() {
  iconKeepAliveTimers.forEach(t => clearTimeout(t));
  iconKeepAliveTimers = [];
}
...
ipcMain.handle('usbip:connect', async (_, host) => {
  // [테스트] USB/IP 비활성화 — 컴2 가상 USB HID 마우스로 테스트
  console.log('[USBIP] 비활성화됨 — 컴2 가상 HID 마우스 테스트 모드');
  currentStreamHost = host;
  return { ok: true, devices: [] };
});
